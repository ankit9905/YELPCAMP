 ...
 router.put("/:comment_id", middleware.checkCommentOwnership, function(req, res) {
         Comment.findByIdAndUpdate(req.params.comment_id,{ useFindAndModify: false }, req.body.comment, function(err, updatedComment) {
             if (err)
                 res.redirect("back");
             else{
 ...
 router.delete("/:comment_id", middleware.checkCommentOwnership, function(req, res) {
     Comment.findByIdAndRemove(req.params.comment_id, { useFindAndModify: false }, function(err) {
         ...
         {
            req.flash("success", "Comment Deleted Succesfullly");
            res.redirect("/campgrounds/" + req.params.id);
         }
     });
